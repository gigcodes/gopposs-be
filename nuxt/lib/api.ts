...
export default class Api {
    ...
    public fetch<Results>(params?: SearchParameters, method = 'GET'): $Fetch<Results, NitroFetchRequest> {
        ...
        return $fetch.create({
            baseURL: this.config.apiURL + this.config.apiPrefix,
            method,
            credentials: 'include',
            params: method === 'GET' ? params : undefined,
            body: ['POST', 'PUT'].includes(method) ? params : undefined,
            headers: {
                Accept: 'application/json', ...(this.token.value && {
                    Authorization: `Bearer ${this.token.value}`,
                })
            },
            onRequest() {
    ...
    public async setUser(): Promise<boolean> {
        try {
            ...
            console.log(this.token.value, e.response._data)
        }
        this.setEcho()
        return true
    }
    ...
    public async attempt(data): Promise<UserLogin> {
        return (await this.fetch<api.MetApiResponse & { data: UserLogin }>(data, 'POST')(`/login`)).data
    }
    ...
    public async logout(): Promise<void> {
        ...
        const response = (await this.fetch<api.MetApiResponse>([], 'POST')('/logout'))
